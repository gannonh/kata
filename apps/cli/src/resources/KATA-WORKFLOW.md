...
| `kata_list_slices`     | List slice issues (parent issues with `kata:slice` label) for a given milestone. Pass `milestoneId` whenever you are working within one milestone.                |
| `kata_list_tasks`      | List task sub-issues for a given slice issue.                                                                                                                      |

### Discovery Rule (Linear mode)

- Use `kata_list_slices({ projectId, teamId, milestoneId })` to enumerate slices for one milestone.
