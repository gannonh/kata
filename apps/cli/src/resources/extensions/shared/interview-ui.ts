 ...
 *
 * Used by /interview-me and /kata-new-project.
 *
      ...
      if (cursorIdx === 1) {
        push(
          ui.actionSelected(
            1,
            opts.satisfiedLabel,
            "Wrap up now and generate the output.",
          ),
        );
      } else {
        push(
      ...
      if (cursorIdx === 0) {
        push(
          ui.actionSelected(
            2,
            opts.keepGoingLabel,
            "Continue with another batch of questions.",
          ),
        );
      } else {
        push(
    ...
    function handleInput(data: string) {
      ...
      if (isMultiQuestion) {
        if (matchesKey(data, Key.left)) {
          switchQuestion(
            (currentIdx - 1 + questions.length) % questions.length,
          );
          return;
        }
      ...
      if (exitCursor === 1) {
        push(
          ui.actionSelected(
            2,
            exitActionLabel,
            "Exit and discard this batch of answers.",
          ),
        );
      } else {
        push(
    ...
    function render(width: number): string[] {
      ...
      for (let i = 0; i < q.options.length; i++) {
        ...
        if (multiSel) {
          ...
          if (isCursor && !focusNotes) {
            push(
              ui.optionSelected(i + 1, opt.label, opt.description, isCommitted),
            );
          } else {
            push(
      ...
      if (focusNotes) {
        ...
        hints.push(
          isLast && allAnswered() ? "enter to review" : "enter to next",
        );
      } else {
        if (st.committedIndex !== null || !isMultiQuestion)
          hints.push("tab to add notes");
