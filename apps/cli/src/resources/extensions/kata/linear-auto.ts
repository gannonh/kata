...
export function buildLinearExecuteTaskPrompt(state: KataState): string {
  ...
  return [
    ...
    ``,
    `9. Write the task summary (scoped to slice issue): \`kata_write_document("${tid}-SUMMARY", content, { issueId: "<slice-issue-uuid>" })\``,
    `   - Include: what shipped (one-liner), what happened, deviations, files modified, verification result.`,
    ``,
    `10. Advance the task to done: \`kata_update_issue_state({ issueId: "<task-uuid>", phase: "done" })\``,
...
export function buildLinearCompleteSlicePrompt(state: KataState): string {
  ...
  return [
    ...
    ``,
    `8. Advance the slice to done: \`kata_update_issue_state({ issueId: "<slice-uuid>", phase: "done" })\``,
    ``,
    REFERENCE,
  ].join("\n");
}
