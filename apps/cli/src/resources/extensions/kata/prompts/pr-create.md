...
The `kata_create_pr` tool handles all pre-flight checks (gh CLI, auth, python3), composes the PR body from slice artifacts, and returns `{ ok: true, url }` on success or `{ ok: false, phase, error, hint }` on failure. Surface any failure details directly to the user.
