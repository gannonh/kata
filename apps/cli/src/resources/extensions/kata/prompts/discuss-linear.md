...
For a new project or any project that does not yet have a `REQUIREMENTS` document in Linear, do a focused research pass before roadmap creation.
...
Before writing a roadmap, produce or update the `REQUIREMENTS` document via `kata_write_document`.
...
4. Call `kata_write_document` with title `{{milestoneId}}-CONTEXT` — read the template at `~/.kata-cli/agent/extensions/kata/templates/context.md` first. Preserve key risks, unknowns, existing codebase constraints, integration points, and relevant requirements surfaced during discussion.
