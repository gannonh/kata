...
<Steps>
  ...
  <Step title="Validate your environment">
    Run doctor to verify harness install, backend config, auth, and backend-specific project requirements:
  ...
  <Step title="Inspect command reference">
    Review the supported command surface in [Commands](/cli/commands) and backend fields in [CLI Preferences](/reference/cli-preferences).
  </Step>
...
Use Linear instead of GitHub Projects v2 with:

<CodeGroup>

```bash npx
npx @kata-sh/cli setup --yes --backend linear --linear-workspace <workspace> --linear-team <team> --linear-project <project>
```
...
<Note>
  Kata stores backend preferences in `.kata/preferences.md`. See [CLI Preferences](/reference/cli-preferences) for all options.
</Note>
