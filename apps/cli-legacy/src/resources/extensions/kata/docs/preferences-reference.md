...
- `github`: GitHub tracker binding. Required when `workflow.mode: github`.
  ...
  - `github.stateMode`: `projects_v2` (required).
  - `github.githubProjectNumber`: positive integer project number (required).
  - `github.labelPrefix`: optional issue label prefix used on Kata-managed artifacts (default `kata:`).
...
---
```
GitHub label mode is removed. Kata requires GitHub Projects v2 for workflow state.
...
github.state_mode: projects_v2
github.githubProjectNumber: 7
validation: valid
```
