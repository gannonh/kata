...
12. If you made an architectural, pattern, library, or observability decision during this task that downstream work should know about, append it to the `DECISIONS` document via `kata_read_document("DECISIONS")` + `kata_write_document("DECISIONS", ...)` (read the template at `~/.kata-cli/agent/extensions/kata/templates/decisions.md` for the format). Not every task produces decisions — only append when a meaningful choice was made.
{{backendOps}}
