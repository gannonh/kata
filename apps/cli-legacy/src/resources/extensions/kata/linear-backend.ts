  ...
  async getIssue(
    ...
    const children: KataIssueRecord[] = includeChildren
      ? (issue.children?.nodes ?? []).map((child) => {
          const childWithOptional = child as typeof child & {
            labels?: Array<{ name: string }>;
          ...
          return {
            ...
            labels: childWithOptional.labels?.map((label: { name: string }) => label.name) ?? [],
            updatedAt: childWithOptional.updatedAt ?? null,
            projectName: issue.project?.name ?? null,
            milestoneName: issue.projectMilestone?.name ?? null,
