...
import { createBackend } from "../resources/extensions/kata/backend-factory.js";
...
function toCanonicalSliceStatus(inputIssue: RuntimeKataIssueRecord): KataSlice["status"] {
  ...
  if (normalizedState === "closed" || normalizedState === "done" || hasPhaseLabel("done")) {
    return "done";
  }
  if (normalizedState === "merging" || hasPhaseLabel("merging")) {
    return "merging";
  }
  if (normalizedState === "human-review" || hasPhaseLabel("human-review")) {
    return "human_review";
  }
  if (normalizedState === "agent-review" || hasPhaseLabel("agent-review")) {
    return "agent_review";
  }
  if (normalizedState === "in-progress" || hasPhaseLabel("in-progress")) {
    return "in_progress";
  }
  return "todo";
}
...
const defaultRuntimeBackendFactory: RuntimeBackendFactory = async (workspacePath) =>
  (await createBackend(workspacePath)) as RuntimeKataBackend;
...
function createRuntimeBackedAdapter(input: {
  ...
  const getBackend = async () => {
    if (!backendPromise) {
      const factory = input.runtimeBackendFactory ?? defaultRuntimeBackendFactory;
...
export async function resolveBackend(input: {
  ...
  if (config.kind === "github") {
    if (input.githubClients) {
      return new GithubProjectsV2Adapter(input.githubClients);
    }
  ...
  if (input.linearClients) {
    return new LinearKataAdapter(input.linearClients);
  }
