...
```bash
INIT=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" init phase-op "$ARGUMENTS")
...
```bash
RESEARCHER_MODEL=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" resolve-model kata-phase-researcher --raw)
```

## 1. Validate Phase

```bash
PHASE_INFO=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" roadmap get-phase "${phase_number}")
```
