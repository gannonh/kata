...
```bash
timestamp=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" current-timestamp full --raw)
```
</step>

<step name="commit">
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/phases/*/.continue-here.md
```
</step>
