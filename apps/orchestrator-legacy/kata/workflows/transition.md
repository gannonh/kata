...
**Delegate ROADMAP.md and STATE.md updates to kata-tools:**

```bash
TRANSITION=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `kata-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" progress bar --raw)
```
...
**Use the transition result from `kata-tools phase complete`:**
...
Exit skill and invoke SlashCommand("/kata:plan-phase [X+1] --auto")
...
Exit skill and invoke SlashCommand("/kata:discuss-phase [X+1] --auto")
...
`/kata:discuss-phase [X+1]` — gather context and clarify approach
...
**Also available:**
- `/kata:plan-phase [X+1]` — skip discussion, plan directly
...
**Also available:**
- `/kata:discuss-phase [X+1]` — revisit context
- `/kata:research-phase [X+1]` — investigate unknowns
...
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" config-set workflow._auto_chain_active false
```
