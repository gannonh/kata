...
```bash
INIT=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" init milestone-op)
...
```bash
integration_checker_model=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" resolve-model kata-integration-checker --raw)
```
...
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" config get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/kata:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/kata:validate-phase {N}` |

Phases needing validation: run `/kata:validate-phase {N}` for each flagged phase.
