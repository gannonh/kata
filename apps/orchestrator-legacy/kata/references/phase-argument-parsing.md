...
```bash
PHASE_INFO=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" find-phase "${PHASE}" --raw)
```
