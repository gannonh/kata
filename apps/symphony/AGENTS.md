...
| `workspace.cleanup_on_done` | bool   | `false`                       | Remove the issue workspace when the issue reaches a terminal state (including startup cleanup for already-terminal issues). Runs `hooks.before_remove` and ignores cleanup failures.         |
