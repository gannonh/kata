...
- When this is part of a Kata-backed Symphony run, use the active Pi/Kata workflow for durable Kata task/slice/artifact state; PR mergeability, reviews, checks, comments, and merge execution remain GitHub operations handled through `gh`.
...
6. Ensure Pi/Symphony agent review comments, Codex review comments (if present), and human review comments are acknowledged and any required fixes are handled before merging.
...
# whether you're addressing or deferring them. Pi/Kata workflow state is not a
...
- All GitHub comments generated by this agent must be prefixed with the active harness identity, e.g. `[pi]` for Pi/Kata CLI runs or `[codex]` for explicit Codex app-server runs. Preserve `[codex]` when replying to existing Codex review comments unless the workflow explicitly standardizes on another prefix.
