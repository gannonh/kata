...
- All GitHub comments generated by this worker must be prefixed with the active runner identity, e.g. `[pi]` for Pi runs or `[codex]` for Codex app-server runs. Preserve `[codex]` when replying to existing Codex review comments unless the workflow explicitly standardizes on another prefix.
