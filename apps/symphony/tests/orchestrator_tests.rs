...
#[test]
fn test_startup_terminal_cleanup_removes_workspace_when_enabled() {
    let workspace_root = tempdir().expect("workspace root should be created");
    let workspace_path = workspace_root.path().join("SIM-105");
    fs::create_dir_all(&workspace_path).expect("workspace should exist before startup cleanup");
    fs::write(workspace_path.join("artifact.txt"), "startup-temp")
        .expect("workspace artifact should exist before startup cleanup");
    ...
    let mut port = FakePort {
        terminal_issues: vec![issue("issue-closed", "SIM-105", "Done", Some(1), 0)],
        ..FakePort::default()
    };
    ...
    assert!(
        ...
        "startup terminal cleanup should remove existing workspace directories when cleanup is enabled"
    );
}
