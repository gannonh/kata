...
| --- | --- | --- |
| R001 — `/` opens autocomplete | `slash-autocomplete.regression.test.tsx` test `[R001]`; `e2e/tests/slash-autocomplete.e2e.ts` test `[R001][R002][R005][R007]` | `s04-01-slash-trigger.png` |
| R002 — builtin command discovery | `command-registry.test.ts` tests `[R002]`; regression `[R002][R003]`; e2e asserts `/kata` visible | `s04-01-slash-trigger.png` |
...
| R005 — ArrowUp/ArrowDown selection routing | regression `[R005]`; e2e verifies selected option changes after `ArrowDown` | `s04-02-arrow-navigation.png` |
| R006 — Tab/Enter accept and insert trailing space | `MessageInput.slash-acceptance.test.tsx`; regression `[R006]` (Enter + Tab + SLASH_ACCEPTED); e2e `[R006]` | `s04-04-tab-accept.png`, `s04-05-enter-accept.png` |
| R007 — Esc dismisses without insertion | regression `[R007]`; e2e `[R001][R002][R005][R007]` | `s04-03-escape-dismiss.png` |
