...
describe('DesktopSessionManager', () => {
  ...
  test('listSessions filters by knownSessionIds when provided', async () => {
    const manager = new DesktopSessionManager(sessionsDir)

    // Create three sessions in the same workspace: desktop, subagent, external CLI
    const desktopPath = path.join(sessionsDir, 'session_desktop-session-id.jsonl')
    const subagentPath = path.join(sessionsDir, 'session_subagent-session-id.jsonl')
    const externalPath = path.join(sessionsDir, 'session_external-session-id.jsonl')

    await writeJsonl(desktopPath, [
      { id: 'desktop-session-id', cwd: workspaceCwd },
      {
        type: 'message',
        message: {
          role: 'user',
    ...
    await writeJsonl(subagentPath, [
      { id: 'subagent-session-id', cwd: workspaceCwd },
      {
        type: 'message',
        message: {
          role: 'user',
          content: 'Task: scout the codebase',
        },
      },
    ])

    await writeJsonl(externalPath, [
      { id: 'external-session-id', cwd: workspaceCwd },
      {
        type: 'message',
        message: {
          role: 'user',
    ...
    // Without knownSessionIds: all three sessions are returned
    const allSessions = await manager.listSessions(workspaceCwd)
    expect(allSessions.sessions).toHaveLength(3)

    // With knownSessionIds: only the desktop session is returned
    const knownIds = new Set(['desktop-session-id'])
    const filtered = await manager.listSessions(workspaceCwd, knownIds)
    expect(filtered.sessions).toHaveLength(1)
    expect(filtered.sessions[0]!.id).toBe('desktop-session-id')
  })

  test('listSessions falls back to showing all when knownSessionIds is empty', async () => {
    const manager = new DesktopSessionManager(sessionsDir)

    await writeJsonl(path.join(sessionsDir, 'session_aaa.jsonl'), [
      { id: 'aaa', cwd: workspaceCwd },
      { type: 'message', message: { role: 'user', content: 'first' } },
    ])

    await writeJsonl(path.join(sessionsDir, 'session_bbb.jsonl'), [
      { id: 'bbb', cwd: workspaceCwd },
      { type: 'message', message: { role: 'user', content: 'second' } },
    ])

    // Empty set: falls back to showing all (startup behavior)
    const result = await manager.listSessions(workspaceCwd, new Set())
    expect(result.sessions).toHaveLength(2)
  })

  test('listSessions with knownSessionIds containing multiple IDs', async () => {
    const manager = new DesktopSessionManager(sessionsDir)

    await writeJsonl(path.join(sessionsDir, 'session_s1.jsonl'), [
      { id: 's1', cwd: workspaceCwd },
      { type: 'message', message: { role: 'user', content: 'session 1' } },
    ])

    await writeJsonl(path.join(sessionsDir, 'session_s2.jsonl'), [
      { id: 's2', cwd: workspaceCwd },
      { type: 'message', message: { role: 'user', content: 'session 2' } },
    ])

    await writeJsonl(path.join(sessionsDir, 'session_s3.jsonl'), [
      { id: 's3', cwd: workspaceCwd },
      { type: 'message', message: { role: 'user', content: 'session 3 (subagent)' } },
    ])

    // User created s1, then new session s2. s3 is a subagent.
    const knownIds = new Set(['s1', 's2'])
    const result = await manager.listSessions(workspaceCwd, knownIds)
    expect(result.sessions).toHaveLength(2)
    expect(result.sessions.map((s) => s.id).sort()).toEqual(['s1', 's2'])
  })

  test('resolveSessionPathById respects knownSessionIds filter', async () => {
    const manager = new DesktopSessionManager(sessionsDir)

    await writeJsonl(path.join(sessionsDir, 'session_owned.jsonl'), [
      { id: 'owned', cwd: workspaceCwd },
      { type: 'message', message: { role: 'user', content: 'owned session' } },
    ])

    await writeJsonl(path.join(sessionsDir, 'session_foreign.jsonl'), [
      { id: 'foreign', cwd: workspaceCwd },
      { type: 'message', message: { role: 'user', content: 'foreign session' } },
    ])

    const knownIds = new Set(['owned'])

    // Can resolve owned session
    const ownedPath = await manager.resolveSessionPathById('owned', workspaceCwd, knownIds)
    expect(ownedPath).toBeTruthy()

    // Cannot resolve foreign session through filtered list
    const foreignPath = await manager.resolveSessionPathById('foreign', workspaceCwd, knownIds)
    expect(foreignPath).toBeNull()

    // Without filter, can resolve foreign session
    const foreignPathUnfiltered = await manager.resolveSessionPathById('foreign', workspaceCwd)
    expect(foreignPathUnfiltered).toBeTruthy()
  })
