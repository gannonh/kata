...
describe('WorkflowBoardService', () => {
  ...
  test('preserves stale github milestone snapshots when filtering removes all cards', async () => {
    const workspacePath = mkdtempSync(path.join(tmpdir(), 'workflow-board-github-milestone-stale-'))
    mkdirSync(path.join(workspacePath, '.kata'), { recursive: true })
    writeFileSync(
      path.join(workspacePath, '.kata', 'preferences.md'),
      [
    ...
    ;(service as any).githubClient.fetchSnapshot = vi.fn(async () => ({
      ...
      emptyReason: 'Using cached stale GitHub project snapshot.',
      poll: { status: 'error', backend: 'github', lastAttemptAt: '2026-04-25T00:00:00.000Z' },
    }))
    ...
    expect(response.snapshot.status).toBe('stale')
    expect(response.snapshot.emptyReason).toBe('Using cached stale GitHub project snapshot.')
    expect(response.snapshot.columns.every((column) => column.cards.length === 0)).toBe(true)
  })
