...
| CRITICAL | <= 25% | Stop immediately, save state (`/kata:pause-work`) |
...
/tmp/claude-ctx-{session_id}.json
    ^ reads
    |
Context Monitor (kata-context-monitor.js, PostToolUse/AfterTool)
    | injects
    v
...
Both hooks are automatically registered during `npx kata-orchestrator` installation:
    ...
    "PostToolUse": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.claude/hooks/kata-context-monitor.js"
          }
        ]
      }
    ]
    ...
    "AfterTool": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.gemini/hooks/kata-context-monitor.js"
          }
        ]
      }
    ]
