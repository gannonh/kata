...
<downstream_consumer>
Your RESEARCH.md is consumed by `kata-planner`:
...
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" websearch "your query" --limit 10
```
...
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
