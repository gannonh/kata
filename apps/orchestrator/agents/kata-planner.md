...
- `/kata:plan-phase --gaps` orchestrator (gap closure from verification failures)
- `/kata:plan-phase` in revision mode (updating plans based on checker feedback)
...
The orchestrator provides user decisions in `<user_decisions>` tags from `/kata:discuss-phase`.
...
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" commit "fix($PHASE): revise plans based on checker feedback" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" history-digest
```
...
```bash
cat "$phase_dir"/*-CONTEXT.md 2>/dev/null   # From /kata:discuss-phase
cat "$phase_dir"/*-RESEARCH.md 2>/dev/null   # From /kata:research-phase
...
Validate each created PLAN.md using kata-tools:

```bash
VALID=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
