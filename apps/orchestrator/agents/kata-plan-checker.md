...
**Question:** Do plans honor user decisions from /kata:discuss-phase?
...
**If missing:** **BLOCKING FAIL** — "VALIDATION.md not found for phase {N}. Re-run `/kata:plan-phase {N} --research` to regenerate."
...
```bash
INIT=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" init phase-op "${PHASE_ARG}")
...
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" roadmap get-phase "$phase_number"
...
Use kata-tools to validate plan structure:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
Extract must_haves from each plan using kata-tools:

```bash
MUST_HAVES=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
Use kata-tools plan-structure verification (already run in Step 2):

```bash
PLAN_STRUCTURE=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
**For manual validation of specificity** (kata-tools checks structure, not content quality):
```bash
...
**DO NOT** check code existence — that's kata-verifier's job. You verify plans, not codebase.
