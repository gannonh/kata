...
const CODEX_AGENT_SANDBOX = {
  ...
  'kata-planner': 'workspace-write',
  'kata-phase-researcher': 'workspace-write',
  'kata-project-researcher': 'workspace-write',
  'kata-research-synthesizer': 'workspace-write',
  'kata-verifier': 'workspace-write',
  'kata-codebase-mapper': 'workspace-write',
  'kata-roadmapper': 'workspace-write',
  'kata-debugger': 'workspace-write',
  'kata-plan-checker': 'read-only',
  'kata-integration-checker': 'read-only',
};
...
const banner = '\n' +
  ...
  '  ╚═╝  ╚═╝╚═╝  ╚═╝   ╚═╝   ╚═╝  ╚═╝' + reset + '\n' +
  '\n' +
  '  Kata Orchestrator ' + dim + 'v' + pkg.version + reset + '\n' +
  '  Multi-agent orchestration and spec-driven development\n' +
...
function convertSlashCommandsToCodexSkillMentions(content) {
  let converted = content.replace(/\/kata:([a-z0-9-]+)/gi, (_, commandName) => {
    return `$kata-${String(commandName).toLowerCase()}`;
  });
  converted = converted.replace(/\/kata-help\b/g, '$kata-help');
  return converted;
}
...
function stripGsdFromCodexConfig(content) {
  ...
  // Remove [agents.kata-*] sections (from header to next section or EOF)
  cleaned = cleaned.replace(/^\[agents\.kata-[^\]]+\]\n(?:(?!\[)[^\n]*\n?)*/gm, '');
...
function mergeCodexConfig(configPath, kataBlock) {
  ...
  if (!fs.existsSync(configPath)) {
    fs.writeFileSync(configPath, kataBlock + '\n');
    return;
  }
  ...
  if (markerIndex !== -1) {
    ...
    if (before) {
      ...
      // Skip [features] from kataBlock if user already has it
      const block = hasFeatures
        ? KATA_CODEX_MARKER + '\n' + kataBlock.substring(kataBlock.indexOf('[agents]'))
        : kataBlock;
  ...
  if (hasFeatures) {
    ...
    // Append agents block (skip the [features] section from kataBlock)
    const agentsBlock = kataBlock.substring(kataBlock.indexOf('[agents]'));
...
function installCodexConfig(targetDir, agentsSrc) {
  ...
  const agentEntries = fs.readdirSync(agentsSrc).filter(f => f.startsWith('kata-') && f.endsWith('.md'));
  const agents = [];
  ...
  const kataBlock = generateCodexConfigBlock(agents);
  mergeCodexConfig(configPath, kataBlock);
...
function convertClaudeToOpencodeFrontmatter(content) {
  ...
  // Replace /kata:command with /kata-command for opencode (flat command structure)
  convertedContent = convertedContent.replace(/\/kata:/g, '/kata-');
 ...
 * OpenCode expects: command/kata-help.md (invoked as /kata-help)
 * Source structure: commands/kata/help.md
 * 
 * @param {string} srcDir - Source directory (e.g., commands/kata/)
 * @param {string} destDir - Destination directory (e.g., command/)
 * @param {string} prefix - Prefix for filenames (e.g., 'kata')
...
function copyFlattenedCommands(srcDir, destDir, prefix, pathPrefix, runtime) {
  ...
  // Remove old kata-*.md files before copying new ones
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // e.g., commands/kata/debug/start.md -> command/kata-debug-start.md
      copyFlattenedCommands(srcPath, destDir, `${prefix}-${entry.name}`, pathPrefix, runtime);
    } else if (entry.name.endsWith('.md')) {
      // Flatten: help.md -> kata-help.md
...
function listCodexSkillNames(skillsDir, prefix = 'kata-') {
...
function cleanupOrphanedFiles(configDir) {
  const orphanedFiles = [
    'hooks/kata-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to .js in v1.9.0
  ];
...
function cleanupOrphanedHooks(settings) {
  const orphanedHookPatterns = [
    'kata-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to .js in v1.9.0
    'kata-intel-index.js',  // Removed in v1.9.2
    'kata-intel-session.js',  // Removed in v1.9.2
    'kata-intel-prune.js',  // Removed in v1.9.2
  ];
  ...
  if (settings.statusLine && settings.statusLine.command &&
    ...
    console.log(`  ${green}✓${reset} Updated statusline path (hooks/statusline.js → hooks/.js)`);
  }
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  if (isOpencode) {
    ...
    if (fs.existsSync(skillsDir)) {
      ...
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
      for (const entry of entries) {
        if (entry.isDirectory() && entry.name.startsWith('kata-')) {
          fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
          skillCount++;
        }
      }
    ...
    if (fs.existsSync(codexAgentsDir)) {
      ...
      for (const file of tomlFiles) {
        if (file.startsWith('kata-') && file.endsWith('.toml')) {
    ...
    // Claude Code & Gemini: remove commands/kata/ directory
    const kataCommandsDir = path.join(targetDir, 'commands', 'kata');
    if (fs.existsSync(kataCommandsDir)) {
      fs.rmSync(kataCommandsDir, { recursive: true });
      removedCount++;
      console.log(`  ${green}✓${reset} Removed commands/kata/`);
    }
  }

  // 2. Remove kata-orchestrator directory
  const kataDir = path.join(targetDir, 'kata-orchestrator');
  if (fs.existsSync(kataDir)) {
    fs.rmSync(kataDir, { recursive: true });
    removedCount++;
          ...
          for (const permType of ['read', 'external_directory']) {
            if (config.permission[permType]) {
              ...
              for (const key of keys) {
                if (key.includes('kata-orchestrator')) {
...
function configureOpencodePermissions(isGlobal = true) {
  ...
  const kataPath = opencodeConfigDir === defaultConfigDir
    ? '~/.config/opencode/kata-orchestrator/*'
    : `${opencodeConfigDir.replace(/\\/g, '/')}/kata-orchestrator/*`;
  ...
  if (config.permission.read[kataPath] !== 'allow') {
    config.permission.read[kataPath] = 'allow';
    modified = true;
  }
  ...
  if (config.permission.external_directory[kataPath] !== 'allow') {
    config.permission.external_directory[kataPath] = 'allow';
    modified = true;
  }
...
const PATCHES_DIR_NAME = 'kata-local-patches';
const MANIFEST_NAME = 'kata-file-manifest.json';
...
function writeManifest(configDir, runtime = 'claude') {
  const isOpencode = runtime === 'opencode';
  const isCodex = runtime === 'codex';
  const kataDir = path.join(configDir, 'kata-orchestrator');
  const commandsDir = path.join(configDir, 'commands', 'kata');
  const opencodeCommandDir = path.join(configDir, 'command');
  const codexSkillsDir = path.join(configDir, 'skills');
  const agentsDir = path.join(configDir, 'agents');
  const manifest = { version: pkg.version, timestamp: new Date().toISOString(), files: {} };

  const kataHashes = generateManifest(kataDir);
  for (const [rel, hash] of Object.entries(kataHashes)) {
    manifest.files['kata-orchestrator/' + rel] = hash;
  }
  if (!isOpencode && !isCodex && fs.existsSync(commandsDir)) {
    const cmdHashes = generateManifest(commandsDir);
    for (const [rel, hash] of Object.entries(cmdHashes)) {
      manifest.files['commands/kata/' + rel] = hash;
    }
  }
...
function install(isGlobal, runtime = 'claude') {
  ...
  // OpenCode uses command/ (flat), Codex uses skills/, Claude/Gemini use commands/kata/
  if (isOpencode) {
    // OpenCode: flat structure in command/ directory
    const commandDir = path.join(targetDir, 'command');
    fs.mkdirSync(commandDir, { recursive: true });
    
    // Copy commands/kata/*.md as command/kata-*.md (flatten structure)
    const kataSrc = path.join(src, 'commands', 'kata');
    copyFlattenedCommands(kataSrc, commandDir, 'kata', pathPrefix, runtime);
    if (verifyInstalled(commandDir, 'command/kata-*')) {
      const count = fs.readdirSync(commandDir).filter(f => f.startsWith('kata-')).length;
      console.log(`  ${green}✓${reset} Installed ${count} commands to command/`);
    } else {
      failures.push('command/kata-*');
    }
  } else if (isCodex) {
    const skillsDir = path.join(targetDir, 'skills');
    const kataSrc = path.join(src, 'commands', 'kata');
    ...
    copyWithPathReplacement(kataSrc, kataDest, pathPrefix, runtime, true);
  ...
  // Copy kata-orchestrator skill with path replacement
  const skillSrc = path.join(src, 'kata');
  const skillDest = path.join(targetDir, 'kata-orchestrator');
  copyWithPathReplacement(skillSrc, skillDest, pathPrefix, runtime);
  if (verifyInstalled(skillDest, 'kata-orchestrator')) {
    console.log(`  ${green}✓${reset} Installed kata-orchestrator`);
  } else {
    failures.push('kata-orchestrator');
  }
  ...
  const changelogDest = path.join(targetDir, 'kata-orchestrator', 'CHANGELOG.md');
  ...
  // Write VERSION file
  const versionDest = path.join(targetDir, 'kata-orchestrator', 'VERSION');
  ...
  const statuslineCommand = isGlobal
    ? buildHookCommand(targetDir, '.js')
    : 'node ' + dirName + '/hooks/.js';
  const updateCheckCommand = isGlobal
    ? buildHookCommand(targetDir, 'kata-check-update.js')
    : 'node ' + dirName + '/hooks/kata-check-update.js';
  const contextMonitorCommand = isGlobal
    ? buildHookCommand(targetDir, 'kata-context-monitor.js')
    : 'node ' + dirName + '/hooks/kata-context-monitor.js';
  ...
  if (!isOpencode) {
    ...
    const hasContextMonitorHook = settings.hooks[postToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('kata-context-monitor'))
    );
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true) {
  ...
  let command = '/kata:new-project';
  if (runtime === 'opencode') command = '/kata-new-project';
  if (runtime === 'codex') command = '$kata-new-project';
  console.log(`
