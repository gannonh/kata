<purpose>
Interactive configuration of Kata workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .planning/config.json with user preferences. Optionally saves settings as global defaults (~/.kata/defaults.json) for future projects.
</purpose>
...
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" config-ensure-section
INIT=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" state load)
...
AskUserQuestion([
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Per Phase", description: "Create branch for each phase (kata/phase-{N}-{name})" },
      { label: "Per Milestone", description: "Create branch for entire milestone (kata/{version}-{name})" }
    ]
  }
])
```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes", description: "New projects start with these settings (saved to ~/.kata/defaults.json)" },
...
If "Yes": write the same config object (minus project-specific fields like `brave_search`) to `~/.kata/defaults.json`:

```bash
mkdir -p ~/.kata
```

Write `~/.kata/defaults.json` with:
```json
{
  "mode": <current>,
  "granularity": <current>,
...
- [ ] User offered to save as global defaults (~/.kata/defaults.json)
