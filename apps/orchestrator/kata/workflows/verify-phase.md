...
<required_reading>
@~/.claude/kata-orchestrator/references/verification-patterns.md
@~/.claude/kata-orchestrator/templates/verification-report.md
</required_reading>
...
Use kata-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
Use kata-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" verify artifacts "$plan")
...
Use kata-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" verify key-links "$plan")
...
See ~/.claude/kata-orchestrator/templates/verification-report.md for complete template.
</step>
