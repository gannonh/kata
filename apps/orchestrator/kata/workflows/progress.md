...
```bash
INIT=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" init progress)
...
<step name="load">
**Use structured extraction from kata-tools:**

Instead of reading full files, use targeted tools to get only the data needed for the report:
- `ROADMAP=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" roadmap analyze)`
- `STATE=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" summary-extract <path> --fields one_liner
  ```
...
<step name="report">
**Generate progress bar from kata-tools, then present rich status report:**

```bash
# Get formatted progress bar
PROGRESS_BAR=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" progress bar --raw)
```
