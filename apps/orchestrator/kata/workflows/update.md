...
for dir in .claude .config/opencode .opencode .gemini; do
  if [ -f "./$dir/kata-orchestrator/VERSION" ]; then
    LOCAL_VERSION_FILE="./$dir/kata-orchestrator/VERSION"
    LOCAL_MARKER_FILE="./$dir/kata-orchestrator/workflows/update.md"
...
for dir in .claude .config/opencode .opencode .gemini; do
  if [ -f "$HOME/$dir/kata-orchestrator/VERSION" ]; then
    GLOBAL_VERSION_FILE="$HOME/$dir/kata-orchestrator/VERSION"
    GLOBAL_MARKER_FILE="$HOME/$dir/kata-orchestrator/workflows/update.md"
...
```bash
npm view kata-orchestrator version 2>/dev/null
```
...
To update manually: `npx kata-orchestrator --global`
```
...
- Custom commands not in `commands/kata/` ✓
...
**If LOCAL install:**
```bash
npx -y kata-orchestrator@latest --local
```

**If GLOBAL install (or unknown):**
```bash
npx -y kata-orchestrator@latest --global
```
...
for dir in .claude .config/opencode .opencode .gemini; do
  rm -f "./$dir/cache/kata-update-check.json"
  rm -f "$HOME/$dir/cache/kata-update-check.json"
done
```

The SessionStart hook (`kata-check-update.js`) writes to the detected runtime's cache directory, so all paths must be cleared to prevent stale update indicators.
</step>
...
[View full changelog](https://github.com/glittercowboy/kata-orchestrator/blob/main/CHANGELOG.md)
```
</step>
...
Check for kata-local-patches/backup-meta.json in the config directory.

**If patches found:**

```
Local patches were backed up before the update.
Run /kata:reapply-patches to merge your modifications into the new version.
```
