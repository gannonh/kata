...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/kata:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.

<step name="initialize" priority="first">
Phase number from argument (required).

```bash
INIT=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" init phase-op "${PHASE}")
...
- options:
  - "Continue and replan after" — Capture context, then run /kata:plan-phase {X} to replan
...
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
```
Skill(skill="kata:plan-phase", args="${PHASE} --auto")
```
...
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ...
  Continue: /kata:execute-phase ${PHASE}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /kata:plan-phase ${PHASE}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /kata:plan-phase ${PHASE} --gaps
  ```
