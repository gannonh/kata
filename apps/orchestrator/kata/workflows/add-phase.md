...
```bash
INIT=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" init phase-op "0")
...
Run /kata:new-project to initialize.
```
Exit.
</step>

<step name="add_phase">
**Delegate the phase addition to kata-tools:**

```bash
RESULT=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" phase add "${description}")
```
...
<success_criteria>
- [ ] `kata-tools phase add` executed successfully
