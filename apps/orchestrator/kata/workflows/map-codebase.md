...
```bash
INIT=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" init map-codebase)
...
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
