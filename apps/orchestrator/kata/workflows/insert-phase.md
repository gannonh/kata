...
Usage: /kata:insert-phase <after> <description>
Example: /kata:insert-phase 72 Fix critical auth bug
```
...
```bash
INIT=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" init phase-op "${after_phase}")
...
**Delegate the phase insertion to kata-tools:**

```bash
RESULT=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" phase insert "${after_phase}" "${description}")
```
...
- [ ] `kata-tools phase insert` executed successfully
