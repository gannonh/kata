...
<template>
@~/.claude/kata-orchestrator/templates/UAT.md
</template>
...
```bash
INIT=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" init verify-work "${PHASE_ARG}")
...
Commit the UAT file:
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
- Follow @~/.claude/kata-orchestrator/workflows/diagnose-issues.md
