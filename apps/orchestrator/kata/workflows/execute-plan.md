...
@~/.claude/kata-orchestrator/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" init execute-phase "${PHASE}")
...
# config settings can be fetched via kata-tools config-get if needed
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="kata-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
2. Per segment:
   - Subagent route: spawn kata-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" phases list --type summaries --raw
...
See `~/.claude/kata-orchestrator/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/kata-orchestrator/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/kata-orchestrator/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/kata-orchestrator/templates/summary.md`.
...
Update STATE.md using kata-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" state update-progress

# Record execution metrics
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" state record-metric \
...
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using kata-tools:

```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" state record-session \
...
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/kata:execute-phase {phase}` + `/kata:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/kata:plan-phase {Z+1}` + `/kata:verify-work {Z}` + `/kata:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/kata:complete-milestone` + `/kata:verify-work` + `/kata:add-phase` |
