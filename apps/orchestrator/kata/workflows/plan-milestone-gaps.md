<purpose>
Create all phases necessary to close gaps identified by `/kata:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/kata:add-phase` per gap.
</purpose>
...
PHASES=$(node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" phases list)
...
```bash
node "$HOME/.claude/kata-orchestrator/bin/kata-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
