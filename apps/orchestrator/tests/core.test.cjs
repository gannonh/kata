...
describe('loadConfig', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'kata-core-test-'));
  ...
  test('returns model_overrides when present (REG-01)', () => {
    writeConfig({ model_overrides: { 'kata-executor': 'opus' } });
    const config = loadConfig(tmpDir);
    assert.deepStrictEqual(config.model_overrides, { 'kata-executor': 'opus' });
  });
...
describe('resolveModelInternal', () => {
  ...
  describe('model profile structural validation', () => {
    test('all known agents resolve to a valid string for each profile', () => {
      const knownAgents = ['kata-planner', 'kata-executor', 'kata-phase-researcher', 'kata-codebase-mapper'];
  ...
  describe('override precedence', () => {
    test('per-agent override takes precedence over profile', () => {
      writeConfig({
        ...
        model_overrides: { 'kata-executor': 'haiku' },
      });
      assert.strictEqual(resolveModelInternal(tmpDir, 'kata-executor'), 'haiku');
    });

    test('opus override resolves to inherit', () => {
      writeConfig({
        model_overrides: { 'kata-executor': 'opus' },
      });
      assert.strictEqual(resolveModelInternal(tmpDir, 'kata-executor'), 'inherit');
    });
    ...
    test('agents not in override fall back to profile', () => {
      ...
      // kata-planner not overridden, should use quality profile -> opus -> inherit
      assert.strictEqual(resolveModelInternal(tmpDir, 'kata-planner'), 'inherit');
    });
  });
  ...
  describe('edge cases', () => {
    test('returns sonnet for unknown agent type', () => {
      writeConfig({ model_profile: 'balanced' });
      assert.strictEqual(resolveModelInternal(tmpDir, 'kata-nonexistent'), 'sonnet');
    });

    test('defaults to balanced profile when model_profile missing', () => {
      writeConfig({});
      // balanced profile, kata-planner -> opus -> inherit
      assert.strictEqual(resolveModelInternal(tmpDir, 'kata-planner'), 'inherit');
    });
  });
});
