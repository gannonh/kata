...
describe('getCodexSkillAdapterHeader', () => {
  test('contains all three sections', () => {
    const result = getCodexSkillAdapterHeader('kata-execute-phase');
  ...
  test('section B maps AskUserQuestion parameters', () => {
    const result = getCodexSkillAdapterHeader('kata-discuss-phase');
...
Run /kata:execute-phase to proceed.`;

    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(result.includes('$kata-execute-phase'), 'converts slash commands');
    assert.ok(!result.includes('/kata:execute-phase'), 'original slash command removed');
  });
...
<role>You are an executor.</role>`;
  ...
  test('sets workspace-write for executor', () => {
    const result = generateCodexAgentToml('kata-executor', sampleAgent);
...
<role>You check plans.</role>`;
    const result = generateCodexAgentToml('kata-plan-checker', checker);
  ...
  test('defaults unknown agents to read-only', () => {
    const result = generateCodexAgentToml('kata-unknown', sampleAgent);
...
describe('CODEX_AGENT_SANDBOX', () => {
  ...
  test('workspace-write agents have write tools', () => {
    const writeAgents = [
      'kata-executor', 'kata-planner', 'kata-phase-researcher',
      'kata-project-researcher', 'kata-research-synthesizer', 'kata-verifier',
      'kata-codebase-mapper', 'kata-roadmapper', 'kata-debugger',
    ];
  ...
  test('read-only agents have no write tools', () => {
    const readOnlyAgents = ['kata-plan-checker', 'kata-integration-checker'];
...
describe('generateCodexConfigBlock', () => {
  const agents = [
    { name: 'kata-executor', description: 'Executes plans' },
  ...
  test('includes per-agent sections', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.includes('[agents.kata-executor]'), 'has executor section');
    assert.ok(result.includes('[agents.kata-planner]'), 'has planner section');
    assert.ok(result.includes('config_file = "agents/kata-executor.toml"'), 'has executor config_file');
...
describe('stripGsdFromCodexConfig', () => {
  ...
  test('strips injected keys above marker on uninstall', () => {
    ...
    assert.ok(!result.includes(KATA_CODEX_MARKER), 'strips marker');
  });
...
describe('mergeCodexConfig', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'kata-codex-merge-'));
  });
  ...
  test('case 1: creates new config.toml', () => {
    ...
    assert.ok(content.includes(KATA_CODEX_MARKER), 'has marker');
  ...
  test('case 2: replaces existing GSD block', () => {
    ...
    const newBlock = generateCodexConfigBlock([
      { name: 'kata-executor', description: 'Updated description' },
      { name: 'kata-planner', description: 'New agent' },
    ]);
    ...
    assert.ok(content.includes('[agents.kata-planner]'), 'has new agent');
    // Verify no duplicate markers
    const markerCount = (content.match(new RegExp(KATA_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
  ...
  test('case 3: appends to config without GSD marker', () => {
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes(KATA_CODEX_MARKER), 'adds marker');
  ...
  test('case 3 with existing [features]: injects keys', () => {
    ...
    assert.ok(content.includes(KATA_CODEX_MARKER), 'adds marker for agents block');
  });
  ...
  test('case 2 strips leaked [agents] from before content', () => {
    ...
    assert.ok(content.includes('[agents.kata-executor]'), 'has agent from fresh block');
  });
...
describe('installCodexConfig (integration)', () => {
  ...
  beforeEach(() => {
    tmpTarget = fs.mkdtempSync(path.join(os.tmpdir(), 'kata-codex-install-'));
  });
  ...
  (hasAgents ? test : test.skip)('generates config.toml and agent .toml files', () => {
    ...
    assert.ok(config.includes('[agents.kata-executor]'), 'has executor agent');

    // Verify per-agent .toml files
    const agentsDir = path.join(tmpTarget, 'agents');
    assert.ok(fs.existsSync(path.join(agentsDir, 'kata-executor.toml')), 'executor .toml exists');
    assert.ok(fs.existsSync(path.join(agentsDir, 'kata-plan-checker.toml')), 'plan-checker .toml exists');

    const executorToml = fs.readFileSync(path.join(agentsDir, 'kata-executor.toml'), 'utf8');
    ...
    const checkerToml = fs.readFileSync(path.join(agentsDir, 'kata-plan-checker.toml'), 'utf8');
