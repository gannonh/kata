...
const WORKFLOWS_DIR = path.join(__dirname, '..', 'kata', 'workflows');
const COMMANDS_DIR = path.join(__dirname, '..', 'commands', 'kata');

const ALL_AGENTS = fs.readdirSync(AGENTS_DIR)
  .filter(f => f.startsWith('kata-') && f.endsWith('.md'))
  .map(f => f.replace('.md', ''));
...
describe('SKILL: skills frontmatter', () => {
  ...
  test('skill references follow naming convention', () => {
    for (const agent of ALL_AGENTS) {
      const content = fs.readFileSync(path.join(AGENTS_DIR, agent + '.md'), 'utf-8');
      const frontmatter = content.split('---')[1] || '';
      const skillLines = frontmatter.split('\n').filter(l => l.trim().startsWith('- kata-'));
      for (const line of skillLines) {
        const skillName = line.trim().replace('- ', '');
        assert.match(skillName, /^kata-[\w-]+-workflow$/, `Invalid skill name: ${skillName}`);
      }
    }
  });
});
...
describe('SPAWN: spawn type consistency', () => {
  test('no "First, read agent .md" workaround pattern remains', () => {
    ...
    for (const dir of dirs) {
      ...
      for (const file of files) {
        ...
        const hasWorkaround = content.includes('First, read ~/.claude/agents/kata-');
        assert.ok(
          !hasWorkaround,
  ...
  test('diagnose-issues uses kata-debugger (not general-purpose)', () => {
    const content = fs.readFileSync(
      path.join(WORKFLOWS_DIR, 'diagnose-issues.md'), 'utf-8'
    );
    assert.ok(
      content.includes('subagent_type="kata-debugger"'),
      'diagnose-issues should spawn kata-debugger, not general-purpose'
    );
  });
});
