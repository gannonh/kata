...
/**
 * Debug script to print the full Kata system prompt with annotations.
...
${colors.bgMagenta}${colors.bold}                       KATA SYSTEM PROMPT BREAKDOWN                              ${colors.reset}
${colors.bgMagenta}${colors.bold}                                                                                ${colors.reset}
`);
...
printAnnotation('  2. Kata Environment Marker - version, platform, arch');
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.kata-cli/agent/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  ...
  '/Users/example/.kata-cli/agent/workspaces/abc123'
);
...
const sessionState = formatSessionState('260121-example-session', {
  plansFolderPath:
    '/Users/example/.kata-cli/agent/workspaces/abc123/sessions/260121-example-session/plans',
});
...
printAnnotation('Generated by the agent source-state formatter - requires agent instance');
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// Kata additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. Kata Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
