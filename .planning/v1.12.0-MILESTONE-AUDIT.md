...
gaps:
  flows:
    - "GAP-1: Stale brownfield summary.md not detected or refreshed — agents receive misleading codebase context when brownfield docs predate significant architecture changes (MAINT-02 partial)"
tech_debt: []
...
## UAT Gap: Stale Brownfield Summary

**Discovered via:** UAT Batch 1, Scenario S1

**Description:** The `.planning/intel/summary.md` in the kata-orchestrator project reflects January 2026 brownfield mapping (GSD-era architecture). The content describes deprecated directories (`commands/`, `agents/`), old naming conventions (`gsd:` prefix), and an architectural model that no longer exists. Agents consuming this summary would receive incorrect codebase context.

**Root cause:** Two design decisions interact to create this blind spot:
1. `update-intel-summary.cjs` guards against overwriting brownfield-generated summary.md (intentional, to preserve richer doc-based summaries)
2. `detect-stale-intel.cjs` checks code-derived index.json entries only — it does not detect when the brownfield mapper docs themselves are stale relative to codebase evolution

**MAINT-02 scope gap:** The requirement ("doc gardening triggers partial re-analysis when documented code areas change significantly") was implemented as code-scan staleness detection, not brownfield doc staleness detection. A project that runs `/kata-map-codebase` once and then evolves significantly has no automatic mechanism to surface that its summary.md is misleading.

**Fix options:**
- A: Add brownfield doc freshness warning: when `.planning/codebase/` docs contain an Analysis Date header older than 30 days (or when major architectural files changed since that date), add a visible stale warning to summary.md
- B: Allow `update-intel-summary.cjs` to regenerate summary from scan data when brownfield docs are older than a threshold, removing the perpetual guard
- C: Surface this in `/kata-map-codebase` output: recommend re-running after significant architectural changes

---

## Recommendations

**v1.12.0 has one gap to close before shipping.**

**GAP-1 must be addressed** — agents consuming stale brownfield summary.md receive incorrect codebase context. This undermines the core value proposition of v1.12.0 (reliable codebase knowledge in agent prompts).

**Next steps:**
1. `/kata-plan-milestone-gaps` — Plan fix phase for GAP-1 (brownfield summary staleness)
2. After fix: `/kata-audit-milestone v1.12.0` — Re-audit to verify gap closed
3. `/kata-complete-milestone v1.12.0` — Archive milestone and ship
