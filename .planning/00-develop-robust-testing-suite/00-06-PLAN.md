...
```bash
test -f tests/skills/debugging.test.js && test -f tests/skills/mapping-codebases.test.js && echo "Files created"
```
...
```bash
test -f tests/skills/starting-projects.test.js && echo "File created"
```
...
After creating the test file, verify it exists:
```bash
test -f tests/skills/updating.test.js && echo "File created"
```
Then run the test (requires ANTHROPIC_API_KEY):
```bash
...
ls -la tests/skills/debugging.test.js tests/skills/mapping-codebases.test.js tests/skills/starting-projects.test.js tests/skills/updating.test.js

# Run all Wave 5 utility skill tests (requires ANTHROPIC_API_KEY)
