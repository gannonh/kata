...
Use config.budgets.quick ($0.50) and config.timeouts.quick (60s) for these simple tests.
  ...
  <verify>
After creating the test file, verify it has correct structure:
```bash
test -f tests/skills/tracking-progress.test.js && head -20 tests/skills/tracking-progress.test.js
```
...
Both use config.budgets.quick and config.timeouts.quick.
  ...
  <verify>
After creating the test files, verify they exist and have correct structure:
```bash
test -f tests/skills/providing-help.test.js && test -f tests/skills/showing-whats-new.test.js && echo "Files created"
```
...
Use config.budgets.standard ($2.00) since this skill may need more context processing.
  ...
  <verify>
After creating the test file, verify it exists and has correct structure:
```bash
test -f tests/skills/listing-phase-assumptions.test.js && echo "File created"
```
Then run the test (requires ANTHROPIC_API_KEY):
```bash
...
ls -la tests/skills/*.test.js

# Run all Wave 1 skill tests (requires ANTHROPIC_API_KEY)
