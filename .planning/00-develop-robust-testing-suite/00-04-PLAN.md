...
Use config.budgets.standard ($2.00) - these modify roadmap structure.
  ...
  <verify>
After creating the test files, verify they exist:
```bash
test -f tests/skills/adding-phases.test.js && test -f tests/skills/inserting-phases.test.js && test -f tests/skills/removing-phases.test.js && echo "Files created"
```
Then run the tests (requires ANTHROPIC_API_KEY):
```bash
...
Use config.budgets.standard ($2.00) - discussion skills involve back-and-forth.
  ...
  <verify>
After creating the test file, verify it exists:
```bash
test -f tests/skills/discussing-phases.test.js && echo "File created"
```
Then run the test (requires ANTHROPIC_API_KEY):
```bash
...
After creating the test files, verify they exist:
```bash
test -f tests/skills/starting-milestones.test.js && test -f tests/skills/completing-milestones.test.js && test -f tests/skills/auditing-milestones.test.js && echo "Files created"
```
Then run the tests (requires ANTHROPIC_API_KEY):
```bash
...
ls -la tests/skills/adding-phases.test.js tests/skills/inserting-phases.test.js tests/skills/removing-phases.test.js tests/skills/discussing-phases.test.js tests/skills/starting-milestones.test.js tests/skills/completing-milestones.test.js tests/skills/auditing-milestones.test.js

# Run all Wave 3 phase/milestone skill tests (requires ANTHROPIC_API_KEY)
