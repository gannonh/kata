...
New (three cases):
```
- When `PR_WORKFLOW=true` AND `WORKTREE_ENABLED=true`: append `\n<working_directory>{WORKTREE_PATH_for_this_plan}</working_directory>` (plan worktree)
- When `PR_WORKFLOW=true` AND `WORKTREE_ENABLED=false`: append `\n<working_directory>{PHASE_WORKTREE_PATH}</working_directory>` (phase worktree)
- When `PR_WORKFLOW=false`: Omit the `<working_directory>` block entirely (existing non-worktree behavior).
```
...
Also update step 10 (commit phase completion) to ensure git operations happen in the phase worktree when PR_WORKFLOW=true. Add a note:

```
    **When PR_WORKFLOW=true:** Git operations in this step execute in the phase worktree. Planning file paths (.planning/) are relative to the worktree root, which has the same directory structure as main/.
```

**Do NOT modify** any other steps or sections.
  </action>
  <verify>
Read SKILL.md step 10.5 and confirm:
1. Uses `$PHASE_BRANCH` not `git branch --show-current`
2. Uses `git -C "$PHASE_WORKTREE_PATH"` for push operations
3. References cleanup-phase for post-merge cleanup
4. `npm run build:plugin` succeeds
5. `npm test` passes
  </verify>
  <done>Step 10.5 pushes and creates PR from phase worktree. No git operations target main/. Cleanup documented for post-merge.</done>
</task>
