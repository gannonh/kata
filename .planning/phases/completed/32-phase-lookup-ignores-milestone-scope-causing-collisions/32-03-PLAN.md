...
2. In the "Current Milestone: v1.6.0 Skills-Native Subagents" section, update phase headers and references:
   ...
   - NOTE: Read actual directory names from `ls` output. Do not assume specific numbers. Plan 01's rename output determines the real values.
...
- NOTE: GitHub issue #102 closure is handled automatically by the PR workflow via the `source_issue` frontmatter field (`source_issue: "github:gannonh/kata-orchestrator#102"`). The PR body will include `Closes #102`. No explicit task needed.
</verification>
