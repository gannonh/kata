...
must_haves:
  truths:
    ...
    - "SKILL.md frontmatter name is kata-customize and description includes trigger phrases: customize template, override template, edit template, list templates, show templates, template overrides, manage templates, template customization, template schema"
    ...
    - "kata-help/SKILL.md includes a /kata-customize entry in the Configuration section"
  artifacts:
    - skills/kata-customize/scripts/list-templates.sh
    - skills/kata-customize/SKILL.md
    - skills/kata-help/SKILL.md
  key_links:
    ...
    - "kata-help Configuration section lists /kata-customize alongside /kata-configure-settings and /kata-set-profile"
---

<objective>
Create the `/kata-customize` skill with a helper script for template discovery. Add a help entry to kata-help.
...
<task type="auto">
  ...
  <files>skills/kata-customize/scripts/list-templates.sh</files>
  <action>
Create the directory structure and helper script.

```bash
mkdir -p skills/kata-customize/scripts
```

Create `skills/kata-customize/scripts/list-templates.sh`:
...
2. Use sibling discovery pattern:
   ...
   ```
   This navigates: `scripts/` -> `kata-customize/` -> `skills/`
...
Make the script executable: `chmod +x skills/kata-customize/scripts/list-templates.sh`
...
[ -x skills/kata-customize/scripts/list-templates.sh ] && echo "OK: executable" || echo "FAIL: not executable"

# No CLAUDE_PLUGIN_ROOT references
grep -c "CLAUDE_PLUGIN_ROOT" skills/kata-customize/scripts/list-templates.sh && echo "FAIL" || echo "OK: no CLAUDE_PLUGIN_ROOT"

# Uses sibling discovery
grep -c 'SCRIPT_DIR/../..' skills/kata-customize/scripts/list-templates.sh | grep -q "[1-9]" && echo "OK: sibling discovery" || echo "FAIL: no sibling discovery"
...
<task type="auto">
  <name>Task 2: Create kata-customize SKILL.md</name>
  <files>skills/kata-customize/SKILL.md</files>
  <action>
Create `skills/kata-customize/SKILL.md` as a single-file orchestrator (no subagents, no references/ directory).
...
```bash
# SKILL.md exists
[ -f skills/kata-customize/SKILL.md ] && echo "OK: SKILL.md exists" || echo "FAIL: no SKILL.md"

# Under 500 lines
LINES=$(wc -l < skills/kata-customize/SKILL.md)
[ "$LINES" -lt 500 ] && echo "OK: ${LINES} lines (under 500)" || echo "FAIL: ${LINES} lines (over 500)"

# Frontmatter has correct name
grep -q "name: kata-customize" skills/kata-customize/SKILL.md && echo "OK: correct name" || echo "FAIL: wrong name"

# Description includes key triggers
grep -q "customize template" skills/kata-customize/SKILL.md && echo "OK: has trigger" || echo "FAIL: missing trigger"
grep -q "list templates" skills/kata-customize/SKILL.md && echo "OK: has trigger" || echo "FAIL: missing trigger"

# References resolve-template.sh
grep -q "resolve-template.sh" skills/kata-customize/SKILL.md && echo "OK: uses resolve-template.sh" || echo "FAIL: missing resolve-template.sh"

# References check-template-drift.sh
grep -q "check-template-drift.sh" skills/kata-customize/SKILL.md && echo "OK: uses check-template-drift.sh" || echo "FAIL: missing check-template-drift.sh"

# References list-templates.sh
grep -q "list-templates.sh" skills/kata-customize/SKILL.md && echo "OK: uses list-templates.sh" || echo "FAIL: missing list-templates.sh"

# No references/ directory
[ -d skills/kata-customize/references ] && echo "FAIL: references/ exists (shouldn't)" || echo "OK: no references/ dir"

# No CLAUDE_PLUGIN_ROOT
grep -c "CLAUDE_PLUGIN_ROOT" skills/kata-customize/SKILL.md && echo "FAIL" || echo "OK: no CLAUDE_PLUGIN_ROOT"
```
  </verify>
...
<task type="auto">
  <name>Task 3: Add kata-customize to kata-help reference</name>
  <files>skills/kata-help/SKILL.md</files>
  <action>
Add an entry for `/kata-customize` to the Configuration section of `skills/kata-help/SKILL.md`.
...
grep -q "kata-customize" skills/kata-help/SKILL.md && echo "OK: help has entry" || echo "FAIL: missing from help"
...
grep -A2 "kata-customize" skills/kata-help/SKILL.md | head -3
```
  </verify>
  <done>kata-help/SKILL.md includes a /kata-customize entry in the Configuration section with usage examples, and .planning/templates/ is listed in the Files & Structure tree.</done>
</task>
...
<success_criteria>
- UI-01: /kata-customize skill exists and responds to natural language triggers
