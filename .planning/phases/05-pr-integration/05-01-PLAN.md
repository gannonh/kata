...
<task type="auto">
  ...
  <action>
    ...
    **Step numbering:** Decimal steps (1.5, 4.5, 10.5) insert between existing integer steps. Steps 2-11 retain their original numbers. Update only the process section list to show new decimal steps in sequence:
    - Step 1: Validate phase exists
    - Step 1.5: Create Phase Branch (NEW)
    - Step 2: Discover plans
    - ...
    - Step 4: Execute waves
    - Step 4.5: Open Draft PR (NEW)
    - ...
    - Step 10: Commit phase completion
    - Step 10.5: Mark PR Ready (NEW)
    - Step 11: Offer next steps

    **Verification:** Confirm step 11 offer_next still correctly references "step 10" for the phase completion context (no renumbering needed for step 11's internal references since step 10 keeps its number).
  </action>
  <verify>grep -A 20 "Create Phase Branch" skills/kata-executing-phases/SKILL.md | grep -q "git checkout -b"</verify>
  <done>Step 1.5 creates phase branch when pr_workflow: true, with re-run protection. Decimal step numbering documented.</done>
</task>
...
PR_EOF
    ...
    **Important:** PR body checklist items remain unchecked throughout execution. The PR body is static after creation — it does NOT update as plans complete. The GitHub issue (updated after each wave in step 4) is the source of truth for plan progress during execution. PR body serves as a summary snapshot, not a live tracker.
  </action>
  <verify>grep -A 30 "Open Draft PR" skills/kata-executing-phases/SKILL.md | grep -q "gh pr create --draft"</verify>
  <done>Step 4.5 opens draft PR after first wave with phase goal and plans checklist. PR body is static; GitHub issue is source of truth for progress.</done>
</task>
