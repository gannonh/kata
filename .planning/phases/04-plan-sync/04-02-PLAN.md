...
<task type="auto">
  ...
  <action>
Modify step 4 (Execute waves) in the `<process>` section of skills/kata-executing-phases/SKILL.md.

**Anchor point:** Insert new step 4.5 AFTER line 79 (`   - Verify SUMMARYs created`) and BEFORE line 80 (`   - Proceed to next wave`).

Add new step as a sibling to the existing bullet points in step 4:
...
The key insight from research: Individual executors run in parallel and would race condition if each tried to update the issue. The orchestrator sees when the wave is done and updates atomically.
  ...
  <verify>
```bash
grep -q "COMPLETED_PLANS_IN_WAVE" skills/kata-executing-phases/SKILL.md && \
grep -q "SUMMARY.md" skills/kata-executing-phases/SKILL.md && \
grep -q "gh issue edit" skills/kata-executing-phases/SKILL.md && \
grep -q "WAVE_PLANS" skills/kata-executing-phases/SKILL.md
```
GitHub issue update logic present with COMPLETED_PLANS_IN_WAVE derivation from SUMMARYs.
  </verify>
  <done>
kata-executing-phases orchestrator includes wave completion hook that:
- Builds COMPLETED_PLANS_IN_WAVE from SUMMARY.md files and WAVE_PLANS tracking
...
After "Goal verified ✓", add conditional output:
```
{If GitHub enabled:}
