...
Add self-assignment in the same GitHub-enabled block:
...
Place this AFTER the label logic, within the same `if [ "$GITHUB_ENABLED" = "true" ]` block.

Apply the same change to the "Work on it now (GitHub-only issue)" section.

Update the confirmation display to show assignment status:
```
...
Update the success_criteria checklist to include:
- [ ] "Work on it now" assigns GitHub Issue to @me (if linked)
  </action>
  <verify>
1. Read skills/check-issues/SKILL.md
2. Verify `gh issue edit.*--add-assignee @me` pattern exists
...
- Both local issue and GitHub-only issue paths have assignment logic
...
1. Check skill file contains `gh issue edit "$ISSUE_NUMBER" --add-assignee @me` command
...
- [ ] Both work-on-it paths (local and GitHub-only) include assignment
