...
<task type="auto">
  <name>Task 1: Create UAT protocol reference document</name>
  <files>skills/kata-verification/references/uat-protocol.md</files>
  <action>
Create new reference document that provides detailed UAT execution guidance.

Extract and adapt from verify-work.md workflow:
1. Test extraction from SUMMARY.md (accomplishments -> testable scenarios)
2. UAT.md file structure and format
3. Test presentation format (checkpoint box style)
4. Response processing (pass/skip/issue with severity inference)
5. Severity inference table (crashes->blocker, doesn't work->major, etc.)
6. File update rules (batched writes, when to persist)
7. Session resumption (finding pending tests)

Include:
- UAT.md template structure
- Example test extraction from accomplishment
- Response processing examples
- Severity inference rules from verify-work.md (lines 536-549)
  </action>
  <verify>File exists at skills/kata-verification/references/uat-protocol.md with >80 lines</verify>
  <done>Reference document provides complete UAT execution guidance</done>
</task>
