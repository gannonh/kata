...
- Display: "Routing to planned execution..."
- Route to planned mode handling (Plan 02 will implement the full flow)

**Why this approach:** Mode selection happens BEFORE moving to in-progress. Quick task moves immediately, planned mode waits for phase linkage.
  </action>
  <verify>
grep -A20 "Work on it now" skills/check-issues/SKILL.md | grep -q "Quick task"
grep -q "Execution Mode" skills/check-issues/SKILL.md
  </verify>
  <done>
- "Work on it now" presents mode selection with Quick task vs Planned options
- Quick task mode routes to execute-quick-task with issue file path
- Planned mode defers to planned execution flow (stub for Plan 02)
  </done>
</task>
...
```bash
# Verify mode selection added
grep -A20 "Work on it now" skills/check-issues/SKILL.md | grep -q "Quick task" && echo "PASS: Mode selection" || echo "FAIL: Mode selection"
...
<success_criteria>
- [ ] EXEC-01: "Work on it now" offers Quick task vs Planned mode selection
